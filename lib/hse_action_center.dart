...
class _HseActionCenterPageState extends State<HseActionCenterPage> {
  ...
  static const String legacyActionsKey = 'workhub_hse_actions_v1';
  static const String legacyMigrationKey = 'safenexus_hse_126_145_migrated_v1';
  ...
  Future<void> _load() async {
    ...
    // One-time migration of the Step 126–145 action/control store.
    // Those steps were 20 near-identical UI variants over the same
    // `workhub_hse_actions_v1` record model. The canonical Action Center
    // already provides the superset of their search, filter, owner,
    // priority, due-date, evidence, verification and closure functions.
    if (!prefs.getBool(legacyMigrationKey)!) {
      final legacyRaw = prefs.getString(legacyActionsKey);
        ...
        try {
          ...
          if (decoded is List) {
            for (final item in decoded.whereType<Map>()) {
              ...
              final status = '${legacy['status'] ?? 'Open'}';
              actions.add({
                'actionNo': '${legacy['actionId'] ?? legacy['id'] ?? ''}',
                'title': '${legacy['actionTitle'] ?? legacy['title'] ?? 'HSE Action'}',
                'owner': '${legacy['assignedTo'] ?? legacy['owner'] ?? legacy['responsiblePerson'] ?? ''}',
                ...
                'raisedDate': '${legacy['createdAt'] ?? ''}',
                'dueDate': '${legacy['dueDate'] ?? legacy['targetDate'] ?? ''}',
                'completedDate': '${legacy['closedAt'] ?? ''}',
                'verificationDate': legacy['closureVerified'] == true
                    ? '${legacy['updatedAt'] ?? ''}'
                    : '',
                'finding': '${legacy['description'] ?? legacy['notes'] ?? ''}',
                'actionDescription': '${legacy['description'] ?? legacy['notes'] ?? ''}',
                'verification': legacy['closureVerified'] == true
                    ? 'Closure verified'
                    : '',
                'evidence': '${legacy['evidence'] ?? ''}',
                'remarks': '${legacy['remarks'] ?? ''}',
                'sourceReference': '${legacy['source'] ?? legacy['module'] ?? ''}',
                'phase': 'Phase 15 - HSE Reporting',
                'project': '',
                'site': '',
                'location': '',
                'raisedBy': '',
                ...
                'auditRef': '',
                'legalRef': '',
                'legacyStep': legacy['step'],
                'legacyModule': legacy['module'],
                'createdAt': '${legacy['createdAt'] ?? DateTime.now().toIso8601String()}',
                'updatedAt': '${legacy['updatedAt'] ?? DateTime.now().toIso8601String()}',
              });
            }
...
// Source: safenexus_step35_action_center.dart
